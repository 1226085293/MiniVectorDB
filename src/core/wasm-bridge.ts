...
const WASM_PATH = (() => {
	const p1 = path.join(__dirname, "../release.wasm"); // dist
	const p2 = path.join(__dirname, "../../build/release.wasm"); // dev/test
	if (fs.existsSync(p1)) return p1;
	if (fs.existsSync(p2)) return p2;
	throw new Error("release.wasm not found (run npm run build:wasm)");
})();
